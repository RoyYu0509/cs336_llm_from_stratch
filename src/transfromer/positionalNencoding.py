...
class PosEncod(nn.Module):
    ...
    def forward(self, x: torch.Tensor, token_positions: torch.Tensor) -> torch.Tensor:
        ...
        """
        x: Float[torch.Tensor, f"Batch,...,seq, d_k"]
        device = x.device
        ...
        # position has the shape: batch, ... so Rs generated by broadcasting will also expand the batch dim.
        Rs: Float[torch.Tensor, f"batch selected_seq_pos, half_d, out_vec, in_vec"]
        Rs = rope_cache[position].to(x.dtype)
            
        # Unpack x into several 2-dim vec
        reshape_x: Float[torch.Tensor, f"Batch, Heads, Seq, half_d, in_vec"]
        ...
        Rs = rearrange(Rs, "bat selected_seq_pos half_d out_vec in_vec -> bat 1 selected_seq_pos half_d out_vec in_vec")
